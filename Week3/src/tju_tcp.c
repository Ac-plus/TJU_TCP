    ...
    while (1)
    {
        if (RETRANS)
        {
            ...
            while (wnd_retrans_size > MAX_DLEN)
            {
                ...
                fprintf(fp, "[SEND] [%ld] [seq:%d ack:0 flag:NO_FLAG]\n", ctime, seq);
                fclose(fp);
                ///////End///////
...
int tju_connect(tju_tcp_t* sock, tju_sock_addr target_addr){
    ...
    fp = fopen("/vagrant/tju_tcp/test/client.event.log", "w");
    fprintf(fp, "[SEND] [%ld] [seq:%d ack:0 flag:SYN]\n", ctime, seq);
    fclose(fp);
    ///////End///////
...
int tju_handle_packet(tju_tcp_t* sock, char* pkt){
    ...
    if (pkt_dst == 1234) {  //根据dst端口号判断包是谁收到的
        if (pkt_flag == SYN_FLAG_MASK) 
            fp = fopen("/vagrant/tju_tcp/test/server.event.log", "w");
        else fp = fopen("/vagrant/tju_tcp/test/server.event.log", "a");
    } else {
        fp = fopen("/vagrant/tju_tcp/test/client.event.log", "a");
    }
    char* log_flag = NULL;
    switch (pkt_flag) {
        case SYN_FLAG_MASK:                 log_flag = "SYN"; break;
        case ACK_FLAG_MASK:                 log_flag = "ACK"; break;
        case SYN_FLAG_MASK|ACK_FLAG_MASK:   log_flag = "SYN|ACK"; break;
        case NO_FLAG:                       log_flag = "NO_FLAG"; break;
        case FIN_FLAG_MASK:                 log_flag = "FIN"; break;
        case FIN_ACK_FLAG_MASK:             log_flag = "FIN|ACK"; break;
        default:                            log_flag = "???"; break;
    }
    fprintf(fp, "[RECV] [%ld] [seq:%d ack:%d flag:%s]\n", ctime, pkt_seq, pkt_ack, log_flag);
    //fputs("This is testing for fputs...\n", fp);
    fclose(fp);
    ///////End///////
    ...
    */
    if (sock->state == LISTEN)
    {
        ...
        if (pkt_flag == SYN_FLAG_MASK)
        {
            ...
            fp = fopen("/vagrant/tju_tcp/test/server.event.log", "a");
            fprintf(fp, "[SEND] [%ld] [seq:%d ack:%d flag:SYN|ACK]\n", ctime, seq, ack);
            //fputs("This is testing for fputs...\n", fp);
            fclose(fp);
            ///////End///////
    ...
    if (sock->state == SYN_SENT)
    {
        if (pkt_ack == CLIENT_CONN_SEQ + 1) //ack==x+1
        {
            ...
            fprintf(fp, "[SEND] [%ld] [seq:%d ack:%d flag:%s]\n", ctime, seq, ack, "ACK");
            //fputs("This is testing for fputs...\n", fp);
            fclose(fp);
            ///////End///////
    ...
    if (sock->state == ESTABLISHED) 
    {
        if (pkt_flag == NO_FLAG)
        {
            ...
            if (pkt_seq == sock->window.wnd_recv->expect_seq)
            {
                ...
                if (sock->established_local_addr.ip == inet_network(SERVER_IP)) {
                    fp1 = fopen("/vagrant/tju_tcp/test/server.event.log", "a");
                } else if (sock->established_local_addr.ip == inet_network(CLIENT_IP)) {
                    fp1 = fopen("/vagrant/tju_tcp/test/client.event.log", "a");
                }
                fprintf(fp1, "[SEND] [%ld] [seq:%d ack:%d flag:ACK]\n", ctime1, seq, ack);
                //fputs("This is testing for fputs...\n", fp);
                fclose(fp1);
                ///////End///////
            ...
            else
            {
                ...
                fprintf(fp, "[SEND] [%ld] [seq:%d ack:%d flag:ACK]\n", ctime, seq, ack);
                //fputs("This is testing for fputs...\n", fp);
                fclose(fp);
                ///////End///////
        ...
        if (pkt_flag == ACK_FLAG_MASK)
        {
            ...
            else if (pkt_ack == sock->window.wnd_send->base)
            {
                ...
                fprintf(fp, "[SWND] [%ld] [size:%d]\n", ctime, sock->window.wnd_send->rwnd);
                //fputs("This is testing for fputs...\n", fp);
                fclose(fp);
                ///////End///////

...
void log_cwnd (tju_tcp_t* sock, int type, uint16_t size)
{
    long ctime = getCurrentTime();
    FILE *fp = NULL;
                
    if (sock->established_local_addr.ip == inet_network(SERVER_IP)) {
        fp = fopen("/vagrant/tju_tcp/test/server.event.trace", "a");
    } else if (sock->established_local_addr.ip == inet_network(CLIENT_IP)) {
        fp = fopen("/vagrant/tju_tcp/test/client.event.trace", "a");
    }

    fprintf(fp, "[%ld] [CWND] [type:%d size:%d]\n", ctime, type, size);
    //fprintf(fp, "[SWND] [] [type: size:]\n");
    fclose(fp);
    fp=NULL;
    return ;
}

void log_swnd (tju_tcp_t* sock) {
    long ctime = getCurrentTime();
    FILE *fp = NULL;           
    if (sock->established_local_addr.ip == inet_network(SERVER_IP)) {
        fp = fopen("/vagrant/tju_tcp/test/server.event.trace", "a");
    ...
    uint16_t size = min(rwnd, cwnd);
    fprintf(fp, "[%ld] [SWND] [size:%d]\n", ctime, size);
    //fprintf(fp, "[SWND] [ctime] [size:1]\n");
    fclose(fp);
    fp = NULL;
    return ;
}

void log_rwnd (tju_tcp_t* sock) {
    long ctime = getCurrentTime();
    FILE *fp = NULL;           
    if (sock->established_local_addr.ip == inet_network(SERVER_IP)) {
        fp = fopen("/vagrant/tju_tcp/test/server.event.trace", "a");
    ...
    uint16_t size = sock->window.wnd_send->rwnd;
    fprintf(fp, "[%ld] [RWND] [size:%d]\n", ctime, size);
    fclose(fp);
    fp = NULL;
    return ;
}
...
tju_tcp_t* tju_socket(){
    ...
    fprintf(fp, "[CWND] [%ld] [type:%d size:%d]\n", ctime, sock->window.wnd_send->congestion_status, sock->window.wnd_send->cwnd);
    fclose(fp);
    fp = NULL;*/
    ///////End///////
    ...
    while (1)
    {
        if (RETRANS)
        {
            ...
            if (TIMEOUT_FLAG)
            {
                ...
                log_cwnd(sock, 3, sock->window.wnd_send->cwnd);
                log_swnd(sock);
                ...
                log_cwnd(sock, 0, sock->window.wnd_send->cwnd);
                log_swnd(sock);
            }
            ...
            while (wnd_retrans_size > MAX_DLEN)
            {
                ...
                fprintf(fp, "[%ld] [SEND] [seq:%d ack:0 flag:0 length:%d]\n", ctime, seq, MAX_DLEN);
                fclose(fp);
                ///////End///////
...
int tju_connect(tju_tcp_t* sock, tju_sock_addr target_addr){
    ...
    fp = fopen("/vagrant/tju_tcp/test/client.event.trace", "w");
    fprintf(fp, "[%ld] [SEND] [seq:%d ack:0 flag:8 length:%d]\n", ctime, seq, 0);
    fclose(fp);
    ///////End///////
...
int tju_handle_packet(tju_tcp_t* sock, char* pkt){
    ...
    if (pkt_dst == 1234) {  //根据dst端口号判断包是谁收到的
        if (pkt_flag == SYN_FLAG_MASK) 
            fp = fopen("/vagrant/tju_tcp/test/server.event.trace", "w");
        else fp = fopen("/vagrant/tju_tcp/test/server.event.trace", "a");
    } else {
        fp = fopen("/vagrant/tju_tcp/test/client.event.trace", "a");
    }
    int log_flag;
    uint16_t pkt_len = get_plen(pkt) - get_hlen(pkt);
    switch (pkt_flag) {
        case SYN_FLAG_MASK:                 log_flag = 8; break;
        case ACK_FLAG_MASK:                 log_flag = 4; break;
        case SYN_FLAG_MASK|ACK_FLAG_MASK:   log_flag = 12; break;
        case NO_FLAG:                       log_flag = 0; break;
        case FIN_FLAG_MASK:                 log_flag = 2; break;
        case FIN_ACK_FLAG_MASK:             log_flag = 6; break;
        default:                            log_flag = 1; break;
    }
    fprintf(fp, "[%ld] [RECV] [seq:%d ack:%d flag:%d length:%d]\n", ctime, pkt_seq, pkt_ack, log_flag, pkt_len);
    //fputs("This is testing for fputs...\n", fp);
    fclose(fp);
    ///////End///////
    ...
    */
    if (sock->state == LISTEN)
    {
        ...
        if (pkt_flag == SYN_FLAG_MASK)
        {
            ...
            fp = fopen("/vagrant/tju_tcp/test/server.event.trace", "a");
            fprintf(fp, "[%ld] [SEND] [seq:%d ack:%d flag:12 length:%d]\n", ctime, seq, ack, 0);
            //fputs("This is testing for fputs...\n", fp);
            fclose(fp);
            ///////End///////
    ...
    if (sock->state == SYN_SENT)
    {
        if (pkt_ack == CLIENT_CONN_SEQ + 1) //ack==x+1
        {
            ...
            fprintf(fp, "[%ld] [SEND] [seq:%d ack:%d flag:4 length:%d]\n", ctime, seq, ack, 0);
            //fputs("This is testing for fputs...\n", fp);
            fclose(fp);
            ///////End///////
    ...
    if (sock->state == ESTABLISHED) 
    {
        if (pkt_flag == NO_FLAG)
        {
            ...
            else if (sock->received_buf != NULL)
            {
                uint32_t data_len = get_plen(pkt) - DEFAULT_HEADER_LEN;
            ...
            if (pkt_seq == sock->window.wnd_recv->expect_seq)
            {
                ...
                fprintf(fp, "[%ld] [RWND] [size:%d]\n", ctime, TCP_BUF_SIZE - sock->received_len - data_len);
                fprintf(fp, "[%ld] [DELV] [seq:%d size:%d]\n", ctime, pkt_seq, get_plen(pkt) - DEFAULT_HEADER_LEN);
                //fputs("This is testing for fputs...\n", fp);
                fclose(fp);
                ///////End///////
                ...
                printf("收到数据报文 seq = %d  发送ACK报文 ack = %d\n", pkt_seq, ack);
                sendToLayer3(msg, DEFAULT_HEADER_LEN + sizeof(struct timeval));
                ///////Logging///////
                long ctime1 = getCurrentTime();
                FILE *fp1 = NULL;
            
                if (sock->established_local_addr.ip == inet_network(SERVER_IP)) {
                    fp1 = fopen("/vagrant/tju_tcp/test/server.event.trace", "a");
                } else if (sock->established_local_addr.ip == inet_network(CLIENT_IP)) {
                    fp1 = fopen("/vagrant/tju_tcp/test/client.event.trace", "a");
                }
                fprintf(fp1, "[%ld] [SEND] [seq:%d ack:%d flag:4 length:%d]\n", ctime1, seq, ack, 0);
                //fputs("This is testing for fputs...\n", fp);
                fclose(fp1);
                ///////End///////
        ...
        if (pkt_flag == ACK_FLAG_MASK)
        {
            ...
            else if (pkt_ack == sock->window.wnd_send->base)
            {
                ...
                sock->window.wnd_send->rwnd = get_advertised_window(pkt);  //对面发给我的建议我应该设置的窗口大小
                log_rwnd(sock);
                log_swnd(sock);
                ...
                if (sock->window.wnd_send->congestion_status == FAST_RECOVERY)
                {
                    ...
                    log_cwnd(sock, 2, sock->window.wnd_send->cwnd);
                    log_swnd(sock);
                ...
                {
                    sock->is_retransing = true;
                    sock->window.wnd_send->ssthresh = sock->window.wnd_send->cwnd / 2; //r->c
            ...
            else
            {
                ...
                else if (sock->window.wnd_send->congestion_status == CONGESTION_AVOIDANCE)
                {
                    ...
                    //log_cwnd(sock, 1, sock->window.wnd_send->cwnd); ////
                    //log_swnd(sock);
        ...
        if (pkt_flag == FIN_ACK_FLAG_MASK)
        {
            ...
            printf("发送 ACK 报文，状态转为 CLOSE_WAIT\n");

            sleep(1);

            //tju_close(sock);  //服务器再次调用
            tju_close_closewait(sock, pkt_seq + 1);
    ...
    if (sock->state == FIN_WAIT_1)
    {
        if (pkt_flag == ACK_FLAG_MASK)
        {
            printf("收到 FIN_ACK 报文，状态转为 FIN_WAIT_2\n");
        ...
        if (pkt_flag == FIN_ACK_FLAG_MASK)
        {
            ...
            // 向客户端发送ACK
            uint32_t seq = sock->window.wnd_send->nextseq + 1;  //为什么是nextseq+1???
            uint32_t ack = pkt_seq + 1;
            char *msg1 = create_packet_buf(sock->established_local_addr.port, sock->established_remote_addr.port, seq, ack,
                                           DEFAULT_HEADER_LEN, DEFAULT_HEADER_LEN, ACK_FLAG_MASK, 1, 0, NULL, 0);
            sendToLayer3(msg1, DEFAULT_HEADER_LEN);
    ...
    if (sock->state == FIN_WAIT_2)
    {
        if (pkt_flag == FIN_ACK_FLAG_MASK)
        {
            ...
            sleep(1);  //Wait for sometime : TIME_WAIT

            printf("发送 ACK 报文，状态转为 CLOSED\n");
            sock->state = CLOSED;
...
int tju_close (tju_tcp_t* sock){
    uint32_t seq = sock->window.wnd_send->nextseq;
    //uint32_t ack = (sock->state == ESTABLISHED) ? 0 : ('?');
    ...
    sendToLayer3(msg, DEFAULT_HEADER_LEN);
    printf("发送 FIN 报文，状态转为 FIN_WAIT_1\n");
    ...
    fprintf(fp, "[%ld] [SEND] [seq:%d ack:%d flag:6 length:%d]\n", ctime, seq, 0, 0);
    fclose(fp);
    ///////End///////
    ...
    else if (sock->state == CLOSE_WAIT)  //这里是server在CLOSE_WAIT状态下再次调用close()
    {
        sock->state = LAST_ACK;
    }
    /*if (sock->established_local_addr.ip==inet_network(SERVER_IP)) {
...
int tju_close_closewait (tju_tcp_t* sock, uint32_t ack){
    uint32_t seq = sock->window.wnd_send->nextseq;
    char *msg = create_packet_buf(sock->established_local_addr.port, sock->established_remote_addr.port, seq, ack,
                                  DEFAULT_HEADER_LEN, DEFAULT_HEADER_LEN, FIN_ACK_FLAG_MASK, 1, 0, NULL, 0);
    
    sendToLayer3(msg, DEFAULT_HEADER_LEN);
    printf("发送 FIN 报文，状态转为 LAST_ACK\n");

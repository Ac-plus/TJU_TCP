...
int TimeoutInterval(tju_tcp_t *sock)
{
    ...
    float srtt = (float)sampleRTT / 1000, dev_rtt = (float)(sock->window.wnd_send->dev_rtt) / 1000;
    float ertt = (float)(sock->window.wnd_send->estmated_rtt) / 1000, t_itv = (float)(sock->window.wnd_send->timeout.it_value.tv_usec) / 1000;
    FILE *fp = NULL;
    ...
    fprintf(fp, "[%ld] [RTTS] [SampleRTT:%f EstimatedRTT:%f DeviationRTT:%f TimeoutInterval:%f]\n", 
...
/******************************以下是改进的连接管理计时器函数*********************************/
void timeout_manage(tju_tcp_t* sock) {
